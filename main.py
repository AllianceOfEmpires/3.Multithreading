...
files_due_to_categories = {'archives': [], 'audio': [],
            ...
            try:
                ...
                if item.is_file():
                    ...
                    if file_type in files_due_to_categories:
                        ...
                        files_due_to_categories[file_type].append(
                            file_name+file_extension)

                    else:

                        files_due_to_categories[file_type] = [
                            file_name+file_extension]
                    ...
                    if file_type == 'archives':
                        ...
                        try:
                            ...
                            shutil.unpack_archive(
                                str(item.resolve()), str(new_dir_path / file_name))
                            item.unlink()
                ...
                if item.is_dir():
                    thread = Thread(target=sorting_files, args=(root_path, item))
                    thread.start()
    ...
    for file_type, list_of_extensions in FILE_TYPES_AND_EXTENSIONS.items():
        ...
        if extension in list_of_extensions:
            ...
            if file_type in types_and_extensions_folder_result:
                ...
                types_and_extensions_folder_result[file_type].update({
                                                                     extension: 1})
                return file_type
